...
export async function execTaskInRepoPod(
  ...
  return withSpan(
    ...
    async () => {
      ...
      const toolchainBootstrap = [
        `if [ -z "\${JAVA_HOME:-}" ]; then`,
        `  for jdk in /home/agent/.local/jdk/* /usr/lib/jvm/java-*-openjdk* /usr/lib/jvm/java-*; do`,
        `    if [ -x "$jdk/bin/java" ]; then`,
        `      export JAVA_HOME="$jdk"`,
        `      export PATH="$JAVA_HOME/bin:$PATH"`,
        `      echo "[optio] JAVA_HOME=$JAVA_HOME"`,
        `      break`,
        `    fi`,
        `  done`,
        ...
        `  export PATH="$JAVA_HOME/bin:$PATH"`,
        `fi`,
        `if [ -z "\${ANDROID_HOME:-}" ] && [ -d /home/agent/Android/Sdk ]; then`,
        `  export ANDROID_HOME=/home/agent/Android/Sdk`,
        `  echo "[optio] ANDROID_HOME=$ANDROID_HOME"`,
        `fi`,
        `if [ -n "\${ANDROID_HOME:-}" ]; then`,
        `  export ANDROID_SDK_ROOT="\${ANDROID_SDK_ROOT:-$ANDROID_HOME}"`,
        `  export PATH="$ANDROID_HOME/platform-tools:$ANDROID_HOME/cmdline-tools/latest/bin:$ANDROID_HOME/tools/bin:$PATH"`,
        `fi`,
        `if [ -n "\${ANDROID_HOME:-}" ] && [ ! -f local.properties ] && { [ -f settings.gradle ] || [ -f settings.gradle.kts ] || [ -f build.gradle ] || [ -f build.gradle.kts ]; }; then`,
        `  if ! git ls-files --error-unmatch local.properties >/dev/null 2>&1; then`,
        `    printf 'sdk.dir=%s\\n' "$ANDROID_HOME" > local.properties`,
        `    echo "[optio] Wrote Android local.properties for SDK discovery"`,
        `  fi`,
        `fi`,
      ];

      const autoPrFallback = [
        `if [ $AGENT_EXIT -eq 0 ] && [ "\${OPTIO_AUTO_CREATE_PR_ON_SUCCESS:-}" = "true" ]; then`,
        `  echo "[optio] Checking for PR after successful agent exit..."`,
        `  if [ "\${OPTIO_GIT_PLATFORM_GITLAB:-}" = "true" ]; then`,
        `    echo "[optio] Auto PR fallback is only implemented for GitHub repos"`,
        ...
        `        OPTIO_PR_BRANCH="\${OPTIO_BRANCH_NAME:-optio/task-$OPTIO_TASK_ID}"`,
        `        git push -u origin "$OPTIO_PR_BRANCH"`,
        `        PR_BODY_FILE="$(mktemp)"`,
        `        {`,
        `          if printf '%s' "\${OPTIO_TICKET_REFERENCE:-}" | grep -Eq '^[0-9]+$'; then`,
        `            echo "Closes #\${OPTIO_TICKET_REFERENCE}"`,
        `          elif [ -n "\${OPTIO_TICKET_REFERENCE:-}" ]; then`,
        `            echo "Related ticket: \${OPTIO_TICKET_REFERENCE}"`,
        `          else`,
        `            echo "Implements task \${OPTIO_TASK_ID}"`,
        `          fi`,
        `          echo`,
        `          echo "Opened automatically by Optio because the agent finished with local changes but did not create a PR."`,
        `        } > "$PR_BODY_FILE"`,
        `        DRAFT_FLAG=""`,
        `        [ "\${OPTIO_DRAFT_PR:-}" = "true" ] && DRAFT_FLAG="--draft"`,
        `        gh pr create --title "\${OPTIO_PR_TITLE:-Implement task $OPTIO_TASK_ID}" --body-file "$PR_BODY_FILE" --base "\${OPTIO_REPO_BRANCH:-main}" --head "$OPTIO_PR_BRANCH" $DRAFT_FLAG || gh pr view --json url -q .url`,
        `        rm -f "$PR_BODY_FILE"`,
        `      else`,
        `        echo "[optio] No local commits to push for auto PR fallback"`,
        `      fi`,
        `    fi`,
        `  else`,
        `    echo "[optio] gh CLI not available; cannot run auto PR fallback"`,
        `  fi`,
        `fi`,
      ];
      ...
      const script = [
        ...
        `if [ -n "\${ANDROID_HOME:-}" ] && ! git ls-files --error-unmatch local.properties >/dev/null 2>&1; then grep -qxF 'local.properties' "$EXCLUDE_FILE" 2>/dev/null || echo 'local.properties' >> "$EXCLUDE_FILE"; fi`,
